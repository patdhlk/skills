...
.. arch-decision:: pds workspace and release shape
   ...
   **Decision.** ``cli/`` is a Cargo workspace from day one:
   ...
   release pipeline is generated by cargo-dist (``dist``), the
   actively maintained axodotdev upstream; the astral-sh fork
   (mentioned at decision time) was archived 2025-12 after its
   changes were upstreamed. Four-target matrix per :need:`ADR_0014`,
